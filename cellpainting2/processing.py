...
class DataSet():
    ...
    def update_similar_refs(self, mode="cpd", write=True):
        ...
        a tsv file is written for use in PPilot.
...
def join_layout_1536(df, plate, quadrant, on="Address_384", sep="\t", how="inner"):
    ...
    result[on] = plate + "-" + quadrant + result["Metadata_Well"]
...
def activity_profile(df, parameters=ACT_PROF_PARAMETERS, act_cutoff=1.58, only_final=True):
    ...
    for key in act_parameters:
        ...
        try:
            result[key] = result[key].apply(lambda x: _log2_mad(x, median, mad))
        except ValueError:
    ...
    if only_final:
        ...
        keep_and_present = [x for x in keep if x in r_keys]
...
def find_similar(df, act_profile, cutoff=0.5, max_num=5, only_final=True,
    ...
    result["Similarity"] = (result[act_parameters]
                            ...
                            .apply(lambda x: cpt.profile_sim(x[0], act_profile)))  # Pandas black belt!!
...
def update_similar_refs(df, mode="cpd", write=True):
    ...
    for _, rec in df.iterrows():
        if rec["Activity"] < LIMIT_ACTIVITY_L or rec["Toxic"]:
            ...
            continue
        act_profile = [rec[x] for x in ACT_PROF_PARAMETERS]
        max_num = 5
        if "ref" in mode:
            max_num += 1
