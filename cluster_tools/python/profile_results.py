...
def profile_plates(plates=None):
    ...
    will be processed)."""
    if plates is None:
        plates = cpt.get_plates_in_dir(cp_config["Dirs"]["Plate"])
    ...
    print("\nPHASE 2: Finding Similar References...")
    # ds_plate.update_similar_refs(write=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--plate", help="Process single plates instead of all data. "
                              "Provide single plate name or comma-separated list.")

    args = parser.parse_args()

    print("Plate:", args.plate)
    profile_plates(plates=args.plate)
